...
@section('title', __('global.cattle_deaths'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.cattle_deaths')}}</h1>
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">{{__('global.cattle_deaths')}}</li>
            </ol>
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('admin.cattle-deaths.create')}}" method="GET">
        ...
        <div class="col-12">
            @can('cattle_death_list')
                <div class="card">
                    <div class="card-header">
                        @can('cattle_death_delete')
                            <a href="{{route('admin.cattle-deaths.trashed')}}" class="btn btn-danger mt-2">{{__('global.trash_list')}}</a>
                        @endcan
                    </div>
                    ...
                    <div class="card-body table-responsive">
                        <table id="adminsList" class="table  dataTable table-bordered table-striped">
                            ...
                            <tbody>
                            <?php $sl = 1; ?>
                            @foreach($cattle_deaths as $cattle_death)
                                <tr>

                                    <td>{{$sl++}}</td>
                                    <td>{{$cattle_death->date}}</td>
                                    <td>{{$cattle_death->cattle->farm->name??'--'}}</td>
                                    <td>{{$cattle_death->cattle->tag_id??'--'}} </td>
                                    <td>{{$cattle_death->amount??0}}</td>
                                    <td>{{__('global.'.$cattle_death->status)}}</td>
                                    <td>{{date_format($cattle_death->updated_at,'d M y h:i A') }}</td>
                                    <td class="text-center">
                                        @can('cattle_death_view')
                                            <a href="{{route('admin.cattle-deaths.show',['cattle_death'=>$cattle_death->id])}}" class="btn btn-info px-1 py-0 btn-sm"><i class="fa fa-eye"></i></a>
                                        @endcan
                                        ...
                                        <form action="{{ route('admin.cattle-deaths.destroy', $cattle_death->id) }}" method="POST">
                                            ...
                                            @can('cattle_death_update')
                                                <a href="{{route('admin.cattle-deaths.edit',['cattle_death'=>$cattle_death->id])}}" class="btn btn-warning px-1 py-0 btn-sm"><i class="fa fa-pen"></i></a>
                                            @endcan
                                            @can('cattle_death_delete')
                                                <button onclick="isDelete(this)" class="btn btn-danger btn-sm px-1 py-0"><i class="fa fa-trash"></i></button>
                                            @endcan
                                            @can('cattle_death_approve')
                                                <a href="{{route('admin.cattle-deaths.approve',['cattle_death'=>$cattle_death->id])}}" class="btn btn-primary btn-sm px-1 py-0"><i class="fa fa-thumbs-up"></i></a>
                                            @endcan
                                        </form>
                                        @else

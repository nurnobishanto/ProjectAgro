...
@section('title', __('global.create_treatment'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{ __('global.create_treatment')}}</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{ __('global.home')}}</a></li>
                <li class="breadcrumb-item"><a href="{{route('admin.treatments.index')}}">{{ __('global.treatments')}}</a></li>
                <li class="breadcrumb-item active">{{ __('global.create_treatment')}}</li>
            </ol>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('admin.treatments.store')}}" method="POST" enctype="multipart/form-data" id="admin-form">
                        @csrf
                        @if (count($errors) > 0)
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <input  name="doctor" id="doctor"  type="text" class="form-control">
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <textarea name="disease" id="disease" rows="1" type="text" class="form-control">{{old('disease')}}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <textarea name="comment" id="comment"  rows="1" type="text" class="form-control">{{old('comment')}}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            @foreach($products as $item)
                                <div class="col-lg-3 col-md-4 col-sm-6">
                                    <div class="card">
                                        <div class="card-body">
                                            <div class="form">
                                                <label>{{ $item->name }} ({{ getStock($cattle->farm_id, $item->id)->quantity ?? 0 }} {{ $item->unit->code }})<span class="text-danger"> *</span></label>

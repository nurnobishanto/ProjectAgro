...
@section('title', __('global.feedings'). __('global.deleted'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.feedings'). __('global.deleted')}}</h1>
            @can('supplier_list')
                <a href="{{route('admin.feedings.index')}}" class="btn btn-primary mt-2">{{__('global.go_back')}}</a>
            @endcan
                        ...
                        <table id="suppliersList" class="table  dataTable table-bordered table-striped">
                            ...
                            @foreach($feedings as $feeding)
                                <tr>
                                    ...
                                    <td>{{date_format($feeding->deleted_at,'d M y h:i A')}}</td>
                                    <td class="text-center">
                                        @can('feeding_delete')
                                        <a href="{{route('admin.feedings.restore',['feeding'=>$feeding->id])}}"  class="btn btn-success btn-sm px-1 py-0"><i class="fa fa-arrow-left"></i></a>
                                        <a href="{{route('admin.feedings.force_delete',['feeding'=>$feeding->id])}}"  class="btn btn-danger btn-sm px-1 py-0"><i class="fa fa-trash"></i></a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach

...
@section('title', __('global.bulk_cattle_sales'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.bulk_cattle_sales')}}</h1>
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">{{__('global.bulk_cattle_sales')}}</li>
            </ol>
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('admin.bulk-cattle-sales.create')}}" method="GET">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-6">
                                <div class="form-group">
                                    @can('bulk_cattle_sale_create')
                                        <input type="submit" value="{{__('global.add_new')}}" class="btn btn-primary form-control mt-md-4 mt-sm-0">
                                    @endcan
                                </div>
                            </div>
                        </div>
        ...
        <div class="col-12">
            @can('bulk_cattle_sale_list')
                <div class="card">
                    <div class="card-header">
                        @can('bulk_cattle_sale_delete')
                            <a href="{{route('admin.bulk-cattle-sales.trashed')}}" class="btn btn-danger mt-2">{{__('global.trash_list')}}</a>
                        @endcan
                    </div>
                    ...
                    <div class="card-body table-responsive">
                        <table id="adminsList" class="table  dataTable table-bordered table-striped">
                            ...
                            <tbody>
                            <?php $sl = 1; ?>
                            @foreach($bulk_cattle_sales as $bulk_cattle_sale)
                                <tr>
                                    ...
                                    <td>{{$bulk_cattle_sale->amount??0}}</td>
                                    <td>{{__('global.'.$bulk_cattle_sale->status)}}</td>
                                    <td>{{date_format($bulk_cattle_sale->updated_at,'d M y h:i A') }}</td>
                                    <td class="text-center">
                                        @can('bulk_cattle_sale_view')
                                            <a href="{{route('admin.bulk-cattle-sales.show',['bulk_cattle_sale'=>$bulk_cattle_sale->id])}}" class="btn btn-info px-1 py-0 btn-sm"><i class="fa fa-eye"></i></a>
                                        @endcan
                                        ...
                                        <form action="{{ route('admin.bulk-cattle-sales.destroy', $bulk_cattle_sale->id) }}" method="POST">
                                            ...
                                            @can('bulk_cattle_sale_update')
                                                <a href="{{route('admin.bulk-cattle-sales.edit',['bulk_cattle_sale'=>$bulk_cattle_sale->id])}}" class="btn btn-warning px-1 py-0 btn-sm"><i class="fa fa-pen"></i></a>
                                            @endcan
                                            @can('bulk_cattle_sale_delete')
                                                <button onclick="isDelete(this)" class="btn btn-danger btn-sm px-1 py-0"><i class="fa fa-trash"></i></button>
                                            @endcan
                                            @can('bulk_cattle_sale_approve')
                                                <a href="{{route('admin.bulk-cattle-sales.approve',['bulk_cattle_sale'=>$bulk_cattle_sale->id])}}" class="btn btn-primary btn-sm px-1 py-0"><i class="fa fa-thumbs-up"></i></a>
                                            @endcan
                                        </form>
                                        @else
                                            <span class="btn btn-info px-1 py-0 btn-sm">Approve</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach

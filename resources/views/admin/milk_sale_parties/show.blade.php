...
@section('title', __('global.view_milk_sale_party'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.view_milk_sale_party')}} - {{$milk_sale_party->name}}</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{__('global.home')}}</a></li>
                <li class="breadcrumb-item"><a href="{{route('admin.milk-sale-parties.index')}}">{{__('global.milk_sale_parties')}}</a></li>
                <li class="breadcrumb-item active">{{__('global.view_milk_sale_party')}}</li>
            </ol>
        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                ...
                                <td><img src="{{asset('uploads/'.$milk_sale_party->photo)}}" alt="{{$milk_sale_party->name}}" class="img-thumbnail" style="max-height: 150px"></td>
                            </tr>
                            <tr>
                                <th>{{__('global.created_by')}}</th>
                                <td>{{$milk_sale_party->createdBy->name}}</td>
                            </tr>
                            <tr>
                                <th>{{__('global.status')}}</th>
                                <td>{{__('global.'.$milk_sale_party->status)}}</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                        <form action="{{ route('admin.milk-sale-parties.destroy', $milk_sale_party->id) }}" method="POST">
                            @method('DELETE')
                            @csrf
                            <a href="{{route('admin.milk-sale-parties.index')}}" class="btn btn-success" >Go Back</a>
                            @can('milk_sale_party_update')
                                <a href="{{route('admin.milk-sale-parties.edit',['milk_sale_party'=>$milk_sale_party->id])}}" class="btn btn-warning "><i class="fa fa-pen"></i> Edit</a>
                            @endcan

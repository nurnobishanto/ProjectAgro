...
@section('title', __('global.feeding_groups'). __('global.deleted'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.feeding_groups'). __('global.deleted')}}</h1>
            @can('supplier_list')
                <a href="{{route('admin.feeding-groups.index')}}" class="btn btn-primary mt-2">{{__('global.go_back')}}</a>
            @endcan
                        ...
                        <table id="suppliersList" class="table  dataTable table-bordered table-striped">
                            ...
                            @foreach($feeding_groups as $feeding_group)
                                <tr>
                                    ...
                                    <td>{{date_format($feeding_group->deleted_at,'d M y h:i A')}}</td>
                                    <td class="text-center">
                                        @can('feeding_group_delete')
                                        <a href="{{route('admin.feeding-groups.restore',['feeding_group'=>$feeding_group->id])}}"  class="btn btn-success btn-sm px-1 py-0"><i class="fa fa-arrow-left"></i></a>
                                        <a href="{{route('admin.feeding-groups.force_delete',['feeding_group'=>$feeding_group->id])}}"  class="btn btn-danger btn-sm px-1 py-0"><i class="fa fa-trash"></i></a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach

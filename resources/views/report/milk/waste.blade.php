...
@section('title', __('global.milk_waste_report'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.milk_waste_report')}}</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{route('admin.dashboard')}}">{{__('global.home')}}</a></li>
                <li class="breadcrumb-item active">{{__('global.milk_waste_report')}}</li>
            </ol>
            ...
            @can('milk_waste_list')
                <div class="card">
                    <div class="card-header">
                        ...
                        <form method="GET" action="{{route('report.milk.waste')}}">
                            @if (count($errors) > 0)
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-2 col-md-3 col-sm-6 ">
                                    <input class="btn-primary btn" type="submit" value="{{__('global.filter')}}">
                                </div>
                            </div>
                        </form>
                        @endcan
                    ...
                    <div class="card-body table-responsive">
                        <table id="adminsList" class="table  dataTable table-bordered table-striped">
                            ...
                            <tr>
                                ...
                                <th class="text-success">{{$milk_wastes->where('status','success')->sum('quantity')}} {{__('global.ltr')}}</th>
                                <th class="text-success"></th>
                                <th class="text-success"></th>
                                <th class="text-success"></th>
                                <th class="text-primary">{{__('global.pending')}} {{__('global.qty')}}</th>
                                <th class="text-primary">{{$milk_wastes->where('status','pending')->sum('quantity')}} {{__('global.ltr')}}</th>

                            </tr>
                            <tr>
                                <th class="text-success">X------X</th>
                                <th class="text-success">{{__('global.success')}}  {{__('global.total')}}</th>
                                <th class="text-success">{{getSetting('currency')}} {{$milk_wastes->where('status','success')->sum('total')}}</th>
                                <th class="text-success"></th>
                                <th class="text-success"></th>
                                <th class="text-success"></th>
                                <th class="text-primary">{{__('global.pending')}} {{__('global.total')}}</th>
                                <th class="text-primary">{{getSetting('currency')}} {{$milk_wastes->where('status','pending')->sum('total')}}</th>
                            </tr>
